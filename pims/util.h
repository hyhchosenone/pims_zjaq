...
/* CRC余式表 */
static unsigned int crc16_xmodem_table[256]
...
{
public:
    static qint8 char2ubyte(char c);
    ...
    static QBitArray bytes2Bits(QByteArray bytes) {
        ...
        QBitArray bits = QBitArray::fromBits(bytes.data(), bytes.size() * 8);

        return bits;
    }

    static QByteArray bits2Bytes(QBitArray bits) {
        QByteArray bytes;
    ...
    static quint8 crcSum(QByteArray data) {
        quint8 sum = 0;
    ...
    //CRC16计算函数
    static quint16 modbusCRC16(QByteArray data)
    {
        int len=data.size();
    ...
    //CCITT CRC16计算函数
    static quint16 CCITT_CRC16(QByteArray data)
    {
        int len = data.size();
        quint16 wcrc = 0x0000; //预置16位crc寄存器，初值全部为0

        int i = 0;                //定义计数
        for (i = 0; i < len; i++) //循环计算每个数据
        {
            quint8 temp = data.at(i);
            wcrc = (wcrc >> 8) ^ crc16_ccitt_table[(wcrc ^ temp) & 0xff];
        }
    ...
    //CCITT CRC16计算函数
    static quint16 CCITT_XMODEM_CRC16(QByteArray data)
    {
        int len = data.size();
        quint16 wcrc = 0x0000; //预置16位crc寄存器，初值全部为0
        ...
        for (int i = 0; i < len; i++) //循环计算每个数据
        {
            ...
            wcrc = (wcrc << 8) ^ crc16_xmodem_table[(wcrc >> 8 ^ temp) & 0xff];
        }

        return wcrc;
    }

    static QByteArray modbusCRC16ToBytes(QByteArray data)
    {
        QByteArray ba;
        ba.resize(2);

        quint16 crc = modbusCRC16(data);
        ba[0] = quint8(crc);
        ba[1] = quint8(crc >> 8);
    ...
    static QByteArray ccitCRC16ToBytes(QByteArray data)
    {
        QByteArray ba;
        ba.resize(2);

        quint16 crc = CCITT_XMODEM_CRC16(data);
        ba[0] = quint8(crc);
        ba[1] = quint8(crc >> 8);
    ...
    static QByteArray buildData(quint8 addr, quint8 cmd, QByteArray ba) {
        QByteArray data;
        data.resize(3);
        data[0] = addr;
        data[1] = ba.length() + 5;
        ...
        data.append(modbusCRC16ToBytes(data));

        return data;
    }

    static QByteArray buildDataV3(quint8 addr, QByteArray ba)
    {
        QByteArray data;
        data.resize(3);
        data[0] = 0;
        ...
        data.append(ccitCRC16ToBytes(data));
